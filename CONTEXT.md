...
| **release gate**    | The time-based rule controlling whether seating data is served. Lives in `lib/exam-release.ts`; every public leg (page shell, `/api/seating/[examId]`) crosses it.                               |
...
- **"gatekeeper"** — the release gate is a module, not a single endpoint; the page shell and the API route both cross it.
